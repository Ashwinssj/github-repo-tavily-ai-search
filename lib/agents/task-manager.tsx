...
export async function taskManager(messages: CoreMessage[]) {
  ...
  try {
    ...
    For example, if the user asks, "Nextjs dashboard with shadcn components?", you may choose to "proceed" as the query is clear and can be answered effectively with web research alone.
    However, if the user asks, "Firebase admin", you may opt to "inquire" and present a form asking about their specific requirements, budget, and preferred features to provide a more tailored recommendation.
